...
def meta_to_dataframes(yaml_data, pretty=False):
    ...
    for chapter, meta in yaml_data.items():
        ...
        if chapter == 'state':
            # make a pretty cell
            # calculate precision
            df['significant_digits'] = [
                (max(0, -round(math.log10(r.resolution)))
            ...
            df['alarms'] = [
                ', '.join([
                    f"[{k}](#{anchor_name(['alarm', k])})"
                    for k in yaml_data['alarm'].keys()
            ...
            df['state'] = [
                markdown_link(['state', vy.alarm_to_state(r.Index)])
                for r in df.itertuples()
            ]
        ...
        # main table
        df['key'] = [
            markdown_anchor([chapter, r.Index]) for r in df.itertuples()
        ]
        dfs[chapter] = df
    return dfs
...
def make_md(yaml_data):
    ...
    # header bar
    md.append(" | ".join(
        [f"*[{snake_to_title(c)}](#{c})*" for c in yaml_data.keys()]))
